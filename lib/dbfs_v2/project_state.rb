...
module DbfsV2
  ...
  class ProjectState
    ...
    # `branch:` a ProjectBranch (or its name) selects the project-branch fold:
    # the branch's own log after its parent's up to the fork, content from the
    # branch's per-file rows, else the parent's at the fork (frozen, like
    # git). `branch_set:` is the older per-file selection over main's log.
    def self.at(store, seq:, branch_set: Branch::MAIN, branch: nil)
      pb = branch.is_a?(ProjectBranch) ? branch : (branch && store.project_branch(branch))
      return at_branch(store, Integer(seq), pb) if pb && !pb.main?
    ...
    def self.at_branch(store, s, pb)
      chain   = pb.lineage(s)                       # [[pb, s], [parent, cut], ..., [main, cut]]
      present = {}
      chain.reverse_each do |b, cut|
        fold_into(present, FileEvent.where(project_branch_id: b.id, seq: ..cut))
      end
      ...
      entries = present.to_h do |id, e|
        name, rev = resolved[id]
        [id, Entry.new(file_node_id: id, path: e[:path], ftype: e[:ftype], branch: name, revision_id: rev)]
      end
      new(project_id: store.project_id, seq: s, branch_set: BranchSet.new(pb.name), entries: entries)
    end
    ...
    class << self
      ...
      def fold_events(project_id, s)
        main_id = ProjectBranch.main_for(project_id).id
        fold_into({}, FileEvent.where(project_id: project_id, project_branch_id: main_id, seq: ..s))
      end

      def fold_into(present, scope)
        scope.order(:seq, :id).pluck(:file_node_id, :kind, :path, :ftype).each do |id, kind, path, ftype|
          case kind
